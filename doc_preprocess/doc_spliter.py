...
def create_bbox_horizontal(origin_span):
    vertical_sorted_span = sorted(
        origin_span, key=lambda span_pos: (span_pos.top, span_pos.left))
    span_count = len(vertical_sorted_span)
    cur_span = vertical_sorted_span[0]
    merge_stage1_spans = []
    for idx in range(1, span_count):
        success, new_span = cur_span.link_horizontal_lines(
            vertical_sorted_span[idx])
        if not success:
    ...
    merge_stage1_spans.append(cur_span)
    vertical_sorted_merge_spans = sorted(
        merge_stage1_spans, key=lambda span_pos: span_pos.left)
    ...
    for idx in range(1, span_count):
        success, new_span = cur_span.merge_horizontal_lines(
            vertical_sorted_merge_spans[idx])
        if not success:
...
def create_bbox_vertical(origin_span):
    horizontal_sorted_span = sorted(
        origin_span, key=lambda span_pos: (span_pos.left, span_pos.top))
    span_count = len(horizontal_sorted_span)
    cur_span = horizontal_sorted_span[0]
    merge_stage1_spans = []
    for idx in range(1, span_count):
        success, new_span = cur_span.link_vertical_lines(
            horizontal_sorted_span[idx])
        if not success:
            merge_stage1_spans.append(cur_span)
    ...
    merge_stage1_spans.append(cur_span)
    horizontal_sorted_merge_spans = sorted(
        merge_stage1_spans, key=lambda span_pos: span_pos.top)
    ...
    for idx in range(1, span_count):
        success, new_span = cur_span.merge_vertical_lines(
            horizontal_sorted_merge_spans[idx])
        if not success:
            merge_stage2_spans.append(cur_span)
...
def find_all_table_bbox(pdf_path):
    ...
    for idx, c in enumerate(table_border):
        ...
        span_pos = {k: int(v[:-2]) for k, v in attr_list if k in ['left',
                                                                  'top', 'width', 'height', 'font-size']}
        keys = span_pos.keys()
        ...
        if span_pos['height'] == 0 and span_pos['width'] > 10:
            h_span.append(Elembbox(
                span_pos['left'], span_pos['top'], span_pos['width'], span_pos['height']))
        if span_pos['width'] == 0 and span_pos['height'] > 10:
            v_span.append(Elembbox(
                span_pos['left'], span_pos['top'], span_pos['width'], span_pos['height']))
    ...
    mbox = []
    mbox.append(merge_bbox)
    lens = len(mbox)
    for i in range(int(lens)):
        mbox[i].margin = mode_font_size
...
# python doc_spliter.py --input_file ./b_data/pdf/2.1.pdf
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--input_file', type=str,
                        default='./1.pdf', help='input file')
    parser.add_argument('--output_dir', type=str,
                        default='.pdf', help='output file')
    parser.add_argument('--sep', type=str, default='=====', help='separtor')
    parser.add_argument('--title_level', type=int, default=4,
                        help='keep the tiltes of level')
    parser.add_argument('--chunk_size', type=int,
                        default=384, help='chunk_size')
    parser.add_argument('--llm_endpoint', type=str,
                        default="", help='llm_endpoint')
